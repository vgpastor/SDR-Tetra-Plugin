...
namespace SDRSharp.Tetra
{
    public unsafe partial class TetraPanel : UserControl
    {
        ...
        {
            if (_iqSamplerate != samplerate)
            {
                _iqSamplerate = samplerate;
                _resampler = new Resampler(_iqSamplerate, 18000 * SamplesPerSymbol);
                tempBuffer?.Dispose();
                tempBuffer = UnsafeBuffer.Create(18000 * SamplesPerSymbol, sizeof(Complex));
                tempBufferPtr = (Complex*)tempBuffer;
            }
            ...
            if (_radioFifoBuffer.Length < 18000 * SamplesPerSymbol)
            {
                int resLength = _resampler.ProcessInterleaved((float*)samples, (float*) tempBufferPtr, length);
                _resampler.ProcessInterleaved(((float*)(samples)) + 1, ((float*)tempBufferPtr) + 1, length);
                _radioFifoBuffer.Write(tempBufferPtr, resLength);
            }
            else
            {
                _lostBuffers++;
        ...
        private void DecodingThread()
        {
            ...
            while (_decodingIsStarted)
            {
                ...
                var audioChannel = _decoder.Process(burst, _outAudioBufferPtr);
        ...
        */
        ...
        private void MarkerTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (_autoSelectChannel)
            {
                ...
                {
                    if (_currentCellLoad[0].IsClear || !GlobalFunction.IgnoreEncryptedSpeech)
                    {
