...
namespace SDRSharp.Tetra
{
    public unsafe partial class TetraPanel : UserControl
    {
        ...
        {
            _iqBuffer = UnsafeBuffer.Create(BurstLengthBits, sizeof(Complex));
            _iqBufferPtr = (Complex*)_iqBuffer;

            _symbolsBuffer = UnsafeBuffer.Create(BurstLengthSymbols + 1, sizeof(float));
            _symbolsBufferPtr = (float*)_symbolsBuffer;

            _diBitsBuffer = UnsafeBuffer.Create(510, sizeof(byte));
            ...
            while (_decodingIsStarted)
            {
                ...
                if ((_radioFifoBuffer == null) || (_radioFifoBuffer.Length < BurstLengthBits) || (_iqSamplerate == 0))
                {
                    Thread.Sleep(10);
                    continue;
                }
                ...
                _radioFifoBuffer.Read(_iqBufferPtr, BurstLengthBits);

                _demodulator.ProcessBuffer(burst, _iqBufferPtr, _iqSamplerate, BurstLengthBits, _symbolsBufferPtr);

                if (burst.Type == BurstType.WaitBurst) continue;
